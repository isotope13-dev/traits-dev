...
CLEAVE_ANALYSIS_MEMO_MB=0 STNG_STRING_CACHE=0 FILEFACTS_CACHE=0 CLEAVE_SKIP_CACHE=1 \
  ...
  testdata/benign/env-http-controls/names-only.js \
...
jq -e -s '
  ...
  all(.[] | select(.raw.files[0].path == "testdata/benign/npm-install-env-unrelated.tgz");
    any(.raw.files[].traits[]?;
      .id == "micro-behaviors/communications/http/request/verb::https-post-option-source")) and
  all($roots[] | select(.path == "testdata/benign/env-http-controls/names-only.js");
    any(.traits[]?; .id == "micro-behaviors/os/env/enumeration::js-object-keys-env") and
    any(.traits[]?; .id == "micro-behaviors/communications/http/request/client::node-http-post-json-body")) and
