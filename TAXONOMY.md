...
> **Visibility caveat — demoting is not a fix, and can be worse than one.** An atomic trait at `component`/`baseline` is emitted only when some composite references it; an unreferenced one is dropped from the JSON output entirely, not merely de-emphasized. (Measured on cleave 2.8.0: `metadata/permission/telemetry::gecko-collects-website-content` matches a Firefox manifest either way, but appears in `--format=json` only at `notable`.) So demotion has two failure modes, and neither is the one you want. If the trait stays referenced, the match is still visible, now mislabeled and keyed to a `directory-path + criticality` ML feature it doesn't belong to. If it is not referenced, the finding silently vanishes — which hides real evidence rather than fixing a false positive, and is how a purpose-defining fact ("this add-on declares it collects authentication data") ends up invisible to the analyst triaging it. Rules are equally important to get right at every criticality level. Lower criticality only when the lower tier is genuinely correct (a true composite fragment or universal-baseline capability). Fix a real false positive properly: tighten the matcher, add an `unless:`/`not:` exclusion, or relocate the trait (see [Matcher Defines Identity](#matcher-defines-identity)).
